...
hrealloc(void* prev, size_t bytes) {
   ...
   //      pthread_mutex_lock(&lock);
	...
	free_block* prev_mem = (free_block*) prev;
	size_t prev_size = prev_mem->size;
	size_t new_size = bytes + sizeof(size_t);
...
// pthread_mutex_unlock(&lock);
		...
		size_t leftover = prev_size - new_size;
		void* excess_loc = prev + leftover;
		free_block* excess_block = (free_block*) excess_loc;
		excess_block->size = leftover;
		excess_block->next = NULL;
		free_list_insert(excess_block);
// pthread_mutex_unlock(&lock);
