...
xrealloc(void* prev, size_t nn)
{
  // TODO: implement a working realloc
	...
	size_t new_size = nn + sizeof(size_t);
	Header* prev_mem = (Header*) (prev - sizeof(size_t));
	size_t prev_size = prev_mem->s.size;
	if (nn == 0) {
		xfree(prev);
		return 0;
	}
		...
		void* new_mem = xmalloc(new_size);
		memcpy(new_mem, prev, prev_mem->s.size);
	        xfree(prev);
		return new_mem;	
	}
	else if (new_size < prev_size) {
		size_t leftover = prev_size - new_size;
		void* excess = prev + leftover;
		xfree(excess);
		return prev;
	}
	else {
